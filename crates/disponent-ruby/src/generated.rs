...
impl Disponent {
    ...
    fn dispatch(&self, args: &[magnus::Value]) -> Result<Session, Error> {
        ...
        let agent_cmd: Option<String> = match args.get(9).copied() {
            Some(v) if !v.is_nil() => Some(magnus::TryConvert::try_convert(v)?),
            _ => None,
        };
        let template: Option<String> = match args.get(10).copied() {
            Some(v) if !v.is_nil() => Some(magnus::TryConvert::try_convert(v)?),
            _ => None,
        };
        let setup: Option<String> = match args.get(11).copied() {
            Some(v) if !v.is_nil() => Some(magnus::TryConvert::try_convert(v)?),
            _ => None,
        };
        let timeout_secs: Option<i32> = match args.get(12).copied() {
            Some(v) if !v.is_nil() => Some(magnus::TryConvert::try_convert(v)?),
            _ => None,
        };
        let max_budget: Option<String> = match args.get(13).copied() {
            Some(v) if !v.is_nil() => Some(magnus::TryConvert::try_convert(v)?),
            _ => None,
        };
        let unchecked: Option<bool> = match args.get(14).copied() {
            Some(v) if !v.is_nil() => Some(magnus::TryConvert::try_convert(v)?),
            _ => None,
        };
        let tags: Option<Vec<String>> = match args.get(15).copied() {
            Some(v) if !v.is_nil() => Some(magnus::TryConvert::try_convert(v)?),
            _ => None,
        };
        let labels: Option<String> = match args.get(16).copied() {
            Some(v) if !v.is_nil() => Some(magnus::TryConvert::try_convert(v)?),
            _ => None,
        };
