...
- The `@worker` decorator + fluessig's `workerHint` lowering (design §13.2), and
  ...
  - **Shipped (declarative, as designed):** `send` and `ack` carry
    `#[fluessig(worker)]` in `crates/disponent-schema`, which the fluessig
    derive front end lowers to `api.json` `"worker": true` → the MCP
    `workerHint` annotation (the sibling of `@readonly`→`readOnlyHint`). The
    gate is `readOnlyHint || workerHint` — the worker-writable set is
    schema-owned, not a hardcoded name list, and moves with the schema.
    - *History:* an interim step shipped a pragmatic **name-based gate**
      (`readOnlyHint || name ∈ {send, ack}`) while fluessig lacked a `worker`
      op flag. fluessig#68 added `#[fluessig(worker)]`; this note now reflects
      the declarative flag that replaced the name list. The worker-role stdio
      server still intercepts the `workerHint` ops to self-scope them (`send`
      recipient-forced to the Manager via `--bound-session`, `ack` confined to
      the bound session's inbox).
